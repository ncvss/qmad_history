# compare our CUDA clover implementation with the clover from qcd_ml called with CUDA
...
for nb in range(0,n_measurements,n_batchlen):
    ...
    for L_incr in range(n_vols):
        ...
        dwc_py = qcd_ml.qcd.dirac.dirac_wilson_clover(Ucu, mass, csw)
        ...
        for n in range(nb,nb+n_batchlen):
            ...
            results[vol]["qcdmlcuda"][n] = stop - start
            ...
            torch.cuda.synchronize()
            stop = time.perf_counter_ns()
            results[vol]["qmadcuda"][n] = stop - start
