...
torch.cuda.synchronize()
res5 = dwc2_f.cu_tsg(vcu)
torch.cuda.synchronize()
res6 = dwc2_sf.cu_tsg_tn(vcu)
...
#time.sleep(5.7)
torch.cuda.synchronize()
print("result differences:", torch.sum(torch.abs(res1cpu-res3cpu)), torch.sum(torch.abs(res1cpu-res4cpu)), torch.sum(torch.abs(res5cpu-res6cpu)))
